...
8) **P-STD-005-CLAUSE-008 (Timeline UID Schema)**
   ...
   * **P-STD-005-CLAUSE-008F (Task UID)** — Task UIDs MUST have the form `<Activity-UID>-TK###`; registered sub-tasks MAY extend the task token using a dotted numeric suffix as `<Activity-UID>-TK###.<n>`, where `<n>` is a positive integer. Regex: `^(?:P|T\\d{3}(?:[A-Z]\\d*)?)-PH\\d{3}-ST\\d{3}-AC\\d{3}(?:\\.\\d+)?-TK\\d{3}(?:\\.\\d+)?$`. Examples: `T102-PH001-ST001-AC009.1-TK003`, `P-PH000-ST001-AC004-TK005.1`.

   * **P-STD-005-CLAUSE-008G (Session UID)** — Session UIDs MUST be expressed by appending `-SES###` to an Activity UID or Task UID. Regex (Activity session): `^(?:P|T\\d{3}(?:[A-Z]\\d*)?)-PH\\d{3}-ST\\d{3}-AC\\d{3}(?:\\.\\d+)?-SES\\d{3}$`. Regex (Task session): `^(?:P|T\\d{3}(?:[A-Z]\\d*)?)-PH\\d{3}-ST\\d{3}-AC\\d{3}(?:\\.\\d+)?-TK\\d{3}(?:\\.\\d+)?-SES\\d{3}$`. Examples: `P-PH000-ST001-AC006-SES003`, `P-PH000-ST001-AC004-TK005.1-SES001`.
