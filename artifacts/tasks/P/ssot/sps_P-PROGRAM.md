...
* **P-STD-001 (Program Governance Standard)** — All combined standard-specification files across `prompt/artifacts/tasks/**` MUST conform to the authoring model defined by this standard's canonical specification. Governs file structure, CLAUSE construction, substandard architecture, decision record format, STD index schema, conformance coupling, specification lifecycle, and the metadata / provenance contract for combined standard files.
  - **Minimum Viable Conformance**: Combined files follow `P-STD-001-CLAUSE-001` canonical structure; normative drafting follows `P-STD-001-CLAUSE-008`; CLAUSEs render per `P-STD-001-CLAUSE-018B`; ADR bodies use `P-STD-001-CLAUSE-025` template; `References` and `Provenance` conform to `P-STD-001-CLAUSE-035` and `P-STD-001-CLAUSE-036`; STD Index entries conform to `P-STD-001-CLAUSE-012A` schema.
  - External Reference: `P-STD-005 (Universal ID Specification)`
...
|:--|:--|:--|:--|
| v0.12.0 | 2026-03-20 | Amendment | Tightened the `P-STD-001` body entry so it stays summary-level while still recording the normative vocabulary and metadata/provenance authority surfaces added by the AC009 recycle remediation pass. |
