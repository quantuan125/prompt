...
|:--|:--|:--|:--|
| CONV-001 | The active gate-time authority for AC004 session-close behavior is this `standards_input` proposal, not the live `prompt/skills/session-close/SKILL.md`. | Restores consultation-only gate purity while keeping the selected architecture explicit. | AC004 comparative analysis; corrected GATE-002 external review |
| CONV-002 | `prompt/skills/session-close/SKILL.md` is preserved for lineage only until TK004 operationalizes the approved convention after gate approval. | Preserves audit traceability without normalizing premature downstream execution. | SES006 corrective session; corrected GATE-002 proposal |
...
|:--|:--|:--|:--|
| RISK-001 | Later authors may still cite the quarantined concrete skill as if it were active gate authority. | medium | Corrected proposal and external review explicitly classify the file as historical/non-authoritative until TK004. |
...
| Corrected External Review | `prompt/artifacts/tasks/P/workspace/PH000/ST002/AC004/analysis/analysis_P-PH000-ST002-AC004-GATE-002_external-review.md` |
