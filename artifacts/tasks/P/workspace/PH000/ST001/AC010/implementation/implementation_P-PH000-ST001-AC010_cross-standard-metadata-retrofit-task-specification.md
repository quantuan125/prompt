...
- Authority chain: AC010 plan authorizes `TK001` through `TK004` -> this artifact specifies HOW those tasks are executed -> future dev-report records execution evidence.
...
| Target file(s) | `prompt/artifacts/tasks/P/standard/standard_P-STD-002_program-status-standard.md` |
...
| Validation check | Re-read the opening frontmatter block, the full `## References` section, and the full `## Provenance` section; confirm only metadata surfaces changed |
...
- `### Amendment History`: preserve the existing dated entries exactly and prepend a new `v1.3.0` entry for the metadata-governance retrofit
