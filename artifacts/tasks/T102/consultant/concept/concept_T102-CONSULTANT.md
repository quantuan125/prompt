...
| ADR ID | Title | Paired GDR | Status | Effective | Supersedes | Anchor |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| `T102-ADR-001` | Consultancy Operating Model | `T102-GDR-001` | Proposed | 2025-09-18 | — | #t102-adr-001-consultancy-model |
| `T102-ADR-002` | Canonical YAML Header          | `T102-GDR-002` | Proposed | 2025-09-14  | —          | #t102-adr-002-yaml-header |
| `T102-ADR-003` | Explicit Inheritance Model     | `T102-GDR-003` | Proposed | 2025-09-14  | —          | #t102-adr-003-explicit-inheritance |
| `T102-ADR-004` | Decision Records Index   | `T102-GDR-004` | Proposed | 2025-09-14  | —          | #t102-adr-004-drs-index |
| `T102-ADR-005` | ID Specification & Rules   | `T102-GDR-005` | Proposed | 2025-09-22  | —          | #t102-adr-005-id-spec |
| `T102-ADR-006` | Research Artifacts Index | `T102-GDR-006` | Proposed | 2025-10-12 | — | #t102-adr-006-research-index |
...
* **T102-ADR-004 (Decision Records Index)** {#t102-adr-004-drs-index}
  ...
  * **Context** 
    Per `T102G-GDR-004`, GDR/ADR schemas and anchors vary across artifacts, causing drift and blocking automation.

  * **Decision** 
    Implement a unified decision record index: one table schema, shared body subheadings, and consistent anchors/links across SPS/Request/Concept/Design.
  ...
  * **Specification**
    ...
    * **T102-ADR-004-CLAUSE-001 (DR Index Schemas)**
      ...
      - **Column Definitions**
        - `ID`: ID construction MUST follow `T102-ADR-005` following general format: `<SID>-<GDR/ADR>-###`
        - `Title`: Title Case, 2–3 words.
        - `Paired GDR` (ADR only): governing GDR ID (or `—` if none).
        - `Status`: `{Proposed, Accepted, Deprecated}`.
        - `Owner`: governance authority (typically `Client`) or implementation owner; if unknown use `—`.
        - `Effective`: ISO-8601 date `YYYY-MM-DD`.
        - `Supersedes`: list of superseded IDs or `—`.
        - `Anchor`: lower-kebab anchor derived from Title; stable.
    ...
    * **T102-ADR-004-CLAUSE-003 (Entry Creation Workflow)**
      ...
      1. Add a new row to the appropriate index table using the required schema (CLAUSE-001).
      2. Assign the next sequential ID for that scope.
      3. Create the matching body entry below the index using CLAUSE-004 structure.
    ...
    * **T102-ADR-004-CLAUSE-004 (DR Body Template)**
      ...
      - **ADR additions** (ADR only):
        - **Specification:** clause list using `CLAUSE-###` (Decision Record Clause IDs / DRCIDs) or software requirement clauses (if used in that ADR).
    ...
    * **T102-ADR-004-CLAUSE-005 (Specification Clauses)**
      ...
      **Normative Reference**
      - `CLAUSE` construction and semantics MUST follow `T102-ADR-005-CLAUSE-005D (Specification Clause Semantics)`.
      ...
      **Non-Duplication Constraint**
      - ADR-004 MUST NOT redefine the syntax, scope validity, or semantic authority of `CLAUSE` IDs; those are defined in `T102-ADR-005-CLAUSE-005D`.
  ...
  * **References** 
    `T102-GDR-004 (Decision Records Standard)`, `T102-IG-007 (ID Standard)`, `T102-IG-008 (Decision Logging)`, `T102-IG-009 (Traceability Framework)`, `T102-GDR-001 (Consultancy Operating Model Standard)`

  * **Provenance** 
    - `design_T102A-SPSST-S3.md`

* **T102-ADR-005 (ID Specification & Rules)** {#t102-adr-005-id-spec}

  * **Context** 
    Per `T102-GDR-005`, multiple artifact families use overlapping ID conventions (considerations, requirements, decisions) at different scopes. Without a single normative spec, authors improvise category tokens, anchors, and references, creating inconsistency and undermining inheritance, precedence, and verification.

  * **Decision** 
    Standardize ID patterns across Initiative, Epic, Feature, and Story scopes; define category tokens, sub-ID patterns, anchors, and referencing rules. This ADR is the single specification adopted by `T102-GDR-005` as policy.
  ...
  * **Specification**
    ...
    * **T102-ADR-005-CLAUSE-002 (Taxonomy & Terminology)**
      ...
      **Category Key**:
      ...
      - `DRCID` (Decision Record Clause ID)
      - `IID` (Implementation Standards / Guidance / Integration Notes)
      - `OID` (Other / Tracking)
      ...
      **Allowed Scope Key**:
      ...
      | `OID` | `ISSUE` | **Open Issue** | I, E, F | Known gap requiring resolution. |
      | `OID` | `RISK` | **Risk** | I, E, F | Potential negative event requiring mitigation. |

    * **T102-ADR-005-CLAUSE-003 (Precedence & Hierarchy)**

      **Directionality**: Inheritance flows downstream only.
      - Correct: Feature references Epic; Epic references Initiative.
      - Incorrect: Epic references Feature (except `INT` exception defined in CLAUSE-005C).

      **Precedence Order** (Highest to Lowest):
      1. Initiative (`I-SID > I-RESID > I-RID > I-DRID`)
      2. Epic (`E-SID > E-RESID > E-RID > E-DRID`)
      3. Feature (`F-SID > F-RESID > F-RID > F-DRID`)
      4. Story (`S-SID > S-RID > S-DRID`) (Story inherits Feature token allowances)

      **Category Precedence** (within same scope):
      `SID > RESID > RID > DRID (GDR > ADR) > DRCID (inherits ADR authority) > IID > OID`
    ...
    * **T102-ADR-005-CLAUSE-005 (Category Semantics)**
      ...
      * **T102-ADR-005-CLAUSE-005A (Assumption Lifecycle)**
        ...
        **Status Enum**:
        `{Pending, Validated, Invalidated, Modified}`
      ...
      * **T102-ADR-005-CLAUSE-005C (Integration Notes Rules)**
        ...
        **Non-Normative Constraint (QA-approved)**:
        ...
        **Scoped Exception (Cross-Feature References Permitted)**:
    ...
    * **T102-ADR-005-CLAUSE-006 (Content Quality)**
      ...
      **Quality Criteria**:
      ...
      - **IID-INT**: Non-normative integration notes; MUST NOT introduce new obligations (see CLAUSE-005C).
      - **DRID**: Follow `T102-ADR-004` body structure strictly.

      **Governance Mapping**:
      In governance-focused artifacts, inner clauses SHOULD be named `CLAUSE` (e.g., `T102-ADR-005-CLAUSE-001`) to prevent confusion with Software Functional Requirements (`FR`). Legacy `...-CLAUSE-###` clause IDs inside governance ADRs are treated as clause IDs during migration.

      **Conciseness**:
      RIDs target <200 words when feasible (excluding tables); IF schemas may exceed for clarity.

    * **T102-ADR-005-CLAUSE-007 (Stability)**
      ...
      - **Migration Tolerance**: Validators MAY allow legacy governance clause labels (e.g., `...-CLAUSE-###` inside governance ADRs) alongside `...-RULE-###` during migration, but new governance clauses SHOULD use `CLAUSE`.

  * **Alternatives Considered**
    - Free-form, template-local ID schemes — rejected (blocks automation and coherent inheritance).
    - Enforcing full titles in all inline mentions — rejected (harms prose readability).
  ...
  * **References** 
    `T102-GDR-005 (ID Governance Standard)`, `T102-ADR-003 (Explicit Inheritance Model)`, `T102-ADR-004 (Decision Records Index)`, `T102-ADR-006 (Research Artifacts Index)`
...
* **T102-ADR-006 (Research Artifacts Index) {#t102-adr-006-research-index}**
  ...
  **Specification**
    ...
    * **T102-ADR-006-CLAUSE-002 (Research Table Schema)**
       ...
       - **Artifact Filenames:**
         - Brief files SHALL follow: `brief_<SCOPE-SID>-<SCOPE-NAME>_<title>_v<MAJOR>.<MINOR>.<PATCH>.md`.
         - Report files SHALL follow: `report_<SCOPE-SID>-<SCOPE-NAME>_<title>_v<MAJOR>.<MINOR>.<PATCH>.md`.
    ...
    * **T102-ADR-006-CLAUSE-007 (NOTE Structure Schema)**
       - Location: SPS "Research & Notes" → "Notes" subheading.
       - Structure: List item per NOTE beginning with `**<NOTE-SID> (<Title>)** — <concise statement>` on a single line.
       - ID pattern: `<SCOPE-SID>-NOTE-###` with sequential numbering per scope.
    ...
    * **T102-ADR-006-CLAUSE-008 (NOTE Authoring Guidelines)**
...
#### 3. Feature ADR Index  
| ADR ID | Title | Status | Related Req IDs | Owning Story | Link |
| :----: | :---- | :----- | :--------------- | :----------- | :--- |
| T102A-SPSST-ADR-0001 | Approvals in body | Accepted | [S1-CLAUSE-002] | S1 | ../design/design_T102A-SPSST-S1.md#adr-0001 |
