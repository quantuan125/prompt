...
| 3 | 3.3 | **Draft `T102-ADR-009` (STD Token Normative Spec)** | `completed` | LLM_Consultant | — | 2026-01-24 | 2026-01-24 | `prompt/artifacts/tasks/T102/consultant/workspace/proposal/proposal_T102-CWD_refactor_gdrs_into_std.md` |
...
| Task ID | Description | Status | Action |
|:--------|:------------|:-------|:-------|
| 2.1.1 | Fix 1-word clause titles violating ADR-005 title constraints | `completed` | Renamed `T102-ADR-005-CLAUSE-007` to `ID Stability & Immutability` |
...
- [x] `T102-STD-009` explicitly references its adopted normative spec (`T102-ADR-009`)
...
**Success Criteria Checklist**:
- [x] `T102-ADR-009` exists and follows the full ADR body template (not Specification-only)
...
| Task ID | Description | Status | Action |
|:--------|:------------|:-------|:-------|
| 4B.1.1 | Draft `T102-STD-005` entry mapping to ADR-005 | `completed` | Added `T102-STD-005` row + body adopting `T102-ADR-005`. |
| 4B.1.2 | Ensure `Adopts` references `T102-ADR-005` using full reference format in index contexts | `completed` | Used full `T102-ADR-005 (ID Specification & Rules)` reference in `Adopts`. |
...
| 4B.3.2 | Deprecate `GDR` with explicit migration tolerance language | `completed` | Referenced migration tolerance via `T102-ADR-009-CLAUSE-005` (kept ADR-005 free of legacy token mentions). |
...
| 5.0.4 | Verify migrated STD entries follow the STD index schema, adoption contract (where applicable), and reference hygiene (shorthand vs full refs) | `completed` | Verified completeness, schema alignment, and reference hygiene; referenced migration tolerance via `T102-ADR-009-CLAUSE-005` without duplication. |
| 5.0.5 | Enhance STD body clarity using “Minimum Viable Conformance” and tighten anti-drift/sync rules in `T102-ADR-009-CLAUSE-004` | `completed` | Updated all `T102-STD-*` bodies to include `Minimum Viable Conformance` (max 5 bullets) and updated `T102-ADR-009-CLAUSE-004` to encode conciseness guidance, drift controls, and same-change-set sync requirements. |
...
- [x] Migration tolerance is referenced via `T102-ADR-009-CLAUSE-005` (do not duplicate tolerance semantics)
