...
**Non-goal**: This activity does NOT perform structural remediation of T102-STDs (AC001 scope), draft verification contracts (AC002), or modify SSOT artifacts (AC004) before GATE-001. The diagnostic and calibration work (TK001–TK007) is reviewed at GATE-001 as a consultation-only recycle gate. Any bounded T102-STD-004 housekeeping mutation and any content remediation seeded by GATE-001 findings are specified first in consultant-owned implementation artifacts (`TK010` / `TK010.1`). `TK010` remains inactive before client approval and becomes the bounded downstream execution contract for the `TK011`-`TK015` implementation package after the same gate records `APPROVE` or `APPROVE WITH CONDITIONS`. `GATE-002` remains pending client disposition until the GDR is recorded.
...
**Objective**: Establish a verified content baseline, structural gap inventory, and supersession current-state assessment for the T102-STD set (GATE-001); then package the same-gate recycle loop with a pre-GATE downstream seed task specification (`TK010`) that later serves as the bounded AC000 execution contract, a recycle remediation specification (`TK010.1`), a same-gate refresh loop (`TK010.2` / `TK010.3`), and the fresh external review / proposal-refresh steps (`TK010.4` / `TK010.5`) while keeping the GATE-002 package ready for client disposition once the live evidence set is complete.
...
| TK010 | `T102-PH001-ST002-AC000-TK010` | Author pre-GATE downstream seed implementation task specification | `completed` | LLM_Consultant | TK009 | `implementation/` | `guideline_workspace_implementation.md` | Task specification authored at `implementation/implementation_T102-PH001-ST002-AC000_tk010_gate-001-downstream-seed-task-specification.md`; execution audience `developer`; inactive before gate approval and then used as the bounded AC000 downstream execution contract for `TK011`-`TK015`. |
| GATE-001 | `T102-PH001-ST002-AC000-GATE-001` | Gate: Client approval of AC000 diagnostic deliverables | `completed` | Client | TK010.5 | Pass/fail | `guideline_workspace_plan.md` §VI.K | Client approved GATE-001 (APPROVE) on 2026-03-31. All four GIR dispositions accepted at option (a). TK010 activated as bounded downstream execution contract for TK011-TK015. |
...
| TK010.3 | `T102-PH001-ST002-AC000-TK010.3` | Review recycle remediation output and refresh Gate-001 package | `completed` | LLM_Consultant | TK010.2 | Refreshed AC000 package for re-presentation | Recycle remediation output | Consultant reviewed the bounded same-gate edits against TK010.1, accepted the refreshed package state, and staged the package for the fresh external review / proposal-refresh loop. |
| TK010.4 | `T102-PH001-ST002-AC000-TK010.4` | Produce fresh second external review for remediated GATE-001 package | `completed` | LLM_Subconsultant (MERS) | TK010.3 | `analysis/` | `guideline_workspace_analysis.md` §IV.B | Fresh second external review published at `analysis/analysis_T102-PH001-ST002-AC000-TK010.4_gate-001-recycle-external-review.md`; TK009 remains historical/outdated evidence only. |
| TK010.5 | `T102-PH001-ST002-AC000-TK010.5` | Assess fresh external review and refresh Gate-001 proposal for client re-presentation | `completed` | LLM_Consultant | TK010.4 | Refreshed Gate-001 proposal package | `guideline_workspace_proposal.md` | Consultant assessed TK010.4, refreshed the Gate-001 proposal for client re-presentation, and recorded that no new developer remediation group is justified without later client approval. |
...
| TK011 | `T102-PH001-ST002-AC000-TK011` | Execute implementation per TK010 downstream execution contract | `completed` | LLM_Developer | GATE-001 | Target STD files and AC000 implementation surfaces named in TK010 | `implementation/implementation_T102-PH001-ST002-AC000_tk010_gate-001-downstream-seed-task-specification.md` | STD-004 normalization executed and validated on disk. |
| TK012 | `T102-PH001-ST002-AC000-TK012` | Produce GATE-002 dev-report | `completed` | LLM_Developer | TK011 | `dev-report/` | `guideline_workspace_dev-report.md` | Primary GATE-002 dev-report authored and indexed. |
| TK013 | `T102-PH001-ST002-AC000-TK013` | Produce GATE-002 verification | `completed` | LLM_Consultant | TK012 | `verification/` | `guideline_workspace_verification.md` | Independent verification completed with `PASS` and no findings. |
| TK014 | `T102-PH001-ST002-AC000-TK014` | Produce GATE-002 gate-disposition proposal | `completed` | LLM_Consultant | TK013 | `proposal/` | `guideline_workspace_proposal.md` | Gate-disposition proposal refreshed for the final client reading set; GDR remains pending. |
| TK015 | `T102-PH001-ST002-AC000-TK015` | Produce GATE-002 external review | `completed` | LLM_Subconsultant | TK014 | `analysis/` | `guideline_workspace_analysis.md` §IV.B | Independent external review completed and incorporated into the gate package. |
| GATE-002 | `T102-PH001-ST002-AC000-GATE-002` | Gate: Client approval of AC000 implementation deliverables | `in_progress` | Client | TK015 | Pass/fail | `guideline_workspace_plan.md` §VI.L | Gate package complete and awaiting client disposition; GDR pending. |
...
**Inputs Required**:
- `prompt/artifacts/tasks/T102/workspace/PH001/ST002/AC000/plan_T102-PH001-ST002-AC000.md` v2.10.0 -- this plan
...
4. Record any residual mutation requirement as explicit recycle-loop remediation input for TK010.1/TK010.5
...
5. Section IV: T102-STD-004 Supersession Status -- current-state posture, residual housekeeping scope for TK010.1/TK010.5, and deferred SPS update
...
2. Assess per `guideline_workspace_analysis.md` §IV.B external review scope requirements:
   ...
   - Downstream task readiness: post-GATE-001 tasks (TK010.1-TK010.5, TK011-TK015, GATE-002) are well-defined and actionable given the analysis findings
...
**Purpose**: Author a detailed developer-facing implementation task specification that seeds the downstream work package for the GATE-001 recycle loop without authorizing execution before approval. This artifact defines the seed boundary, the blocked downstream handoff, and the bounded execution contract that later governs `TK011`-`TK015` after the gate passes.
...
**Scope**:
- In scope: Downstream task seeding for the recycle package; implementation audience and handoff boundary definition; bounded edit scope for later assistant execution; AC000-local downstream execution contract for `TK011`-`TK015`
...
2. Extract implementation items from the gate package that must be seeded for the assistant sub-agent and later executed under AC000:
   - the later STD-004 housekeeping mutation that remains blocked until same-gate approval
   - any AC000-scoped residual content remediation that the recycle package needs to seed downstream
   - the traceability expectations that `TK012`-`TK015` must inherit from this execution contract
3. Author the task specification per `guideline_workspace_implementation.md` with exact SPEC items, developer audience, do-not-execute-yet constraints before gate approval, and activation rules after gate approval

**Success Criteria**:
- [ ] Seed implementation spec exists with SPEC items for all downstream AC000 work that must be staged and later executed
- [ ] Each SPEC item has target files, end-state posture, validation checks, and blocking ambiguity rules
- [ ] The artifact explicitly states that it becomes the bounded execution contract for `TK011`-`TK015` after gate approval
...
**Entry Criteria**:
- TK001 through TK010.5 are completed (content verification, structural gap assessment, supersession current-state assessment, calibrated scope brief, pre-GATE seed task specification, recycle remediation specification, same-gate refresh loop, fresh external review, and proposal refresh)
...
- If APPROVE: AC001-AC004 may proceed using the calibrated scope brief as primary diagnostic input; AC000 post-GATE-001 implementation may begin only through `TK011` executing against `TK010`
- If APPROVE WITH CONDITIONS: conditions are recorded and govern downstream sequencing before AC001 begins or before later post-gate implementation starts; AC000 post-gate implementation remains governed by `TK010`
- If RECYCLE: same-gate remediation tasks TK010.1-TK010.5 are activated, the package is re-presented under the same gate ID, and TK011-TK015 / GATE-002 remain blocked until a later client approval decision
...
**External Review (fresh second-opinion)**: `prompt/artifacts/tasks/T102/workspace/PH001/ST002/AC000/analysis/analysis_T102-PH001-ST002-AC000-TK010.4_gate-001-recycle-external-review.md`
...
**Recycle Trigger**: Current package review returns `RECYCLE` because the live AC000 package still needs same-gate package correction and re-presentation, including a fresh second external review and consultant proposal refresh before client re-presentation. The current external review is retained only as historical/outdated evidence.
**Remediation Tasks**: `TK010.1` (author GATE-001 recycle remediation specification), `TK010.2` (assistant sub-agent executes bounded same-gate edits), `TK010.3` (consultant reviews output and refreshes the package for re-presentation), `TK010.4` (fresh second external review for the remediated package), `TK010.5` (consultant assesses the fresh review and refreshes the proposal for client re-presentation)
**Re-entry Criteria**: TK010.1-TK010.5 complete; AC000 plan/proposal/stream plan all reflect the re-presented post-recycle package; current external review remains historical-only; fresh second external review and consultant assessment are present; downstream blocked tasks are still not released.
...
**Downstream Block**: `TK011`-`TK015` and `GATE-002` remain blocked until the same gate later records a client approval decision after TK010.5. That approval activates `TK010` as the bounded downstream execution contract. If TK010.4 or TK010.5 identify new remediation needs, those needs are first presented to the Client before any new developer remediation group is commissioned.
...
**Purpose**: Review the assistant output for spec compliance, refresh the gate package for same-gate re-presentation, and preserve the historical/outdated status of the current external review while staging the package for TK010.4/TK010.5.

**Deliverable**:
- Refreshed AC000 gate package staged for fresh external review and later client re-presentation

**Scope**:
- In scope: Review of the assistant-sub-agent output; package refresh; gate-package traceability updates; handoff into the fresh external review / proposal-refresh loop
...
2. Refresh the package surfaces needed for re-presentation and hand off to TK010.4/TK010.5.
3. Confirm the current external review remains evidence-only and the next review step is still uncommissioned in this session.

**Success Criteria**:
- [ ] Package is internally consistent for the recycle loop and staged for TK010.4/TK010.5
- [ ] No new external review has been commissioned

### Task TK010.4: Produce Fresh Second External Review for Remediated GATE-001 Package

**Task ID**: `T102-PH001-ST002-AC000-TK010.4`

**Purpose**: Produce the fresh second-opinion review for the remediated GATE-001 package so the consultant can assess the refreshed package before re-presentation.

**Deliverable**:
- `prompt/artifacts/tasks/T102/workspace/PH001/ST002/AC000/analysis/analysis_T102-PH001-ST002-AC000-TK010.4_gate-001-recycle-external-review.md`

**Scope**:
- In scope: Review of the refreshed TK010.3 package; assessment of evidence integrity, role boundaries, plan compliance, re-presentation readiness, and any residual issues needing later client decision
- Out of scope: Re-performing the package repair; authorizing a new developer remediation group; overriding proposal authority

**Inputs Required**:
- Refreshed AC000 package from TK010.3
- `prompt/templates/consultant/workspace/guideline_workspace_analysis.md` -- external review authoring rules
- `prompt/artifacts/tasks/T102/workspace/PH001/ST002/AC000/analysis/analysis_T102-PH001-ST002-AC000-TK009_gate-001-external-review.md` -- historical/outdated review for traceability only

**Steps**:
1. Read the refreshed AC000 package and the historical TK009 review.
2. Assess evidence integrity, role boundaries, plan compliance, re-presentation readiness, and any residual issues.
3. Record findings without inventing follow-on remediation scope.
4. Leave proposal refresh and client re-presentation packaging to TK010.5.

**Success Criteria**:
- [ ] Fresh external review artifact exists at the specified path
- [ ] All assessment criteria are addressed
- [ ] No implementation work or new developer remediation group has been created prematurely

### Task TK010.5: Assess Fresh External Review and Refresh Gate-001 Proposal for Client Re-Presentation

**Task ID**: `T102-PH001-ST002-AC000-TK010.5`

**Purpose**: Perform the main consultant assessment of the fresh external review and refresh the Gate-001 proposal for client re-presentation without expanding the remediation scope.

**Deliverable**:
- Refreshed Gate-001 proposal package ready for client re-presentation

**Scope**:
- In scope: Assessment of TK010.4 findings; proposal refresh; traceability updates; re-presentation packaging; deferral of any further remediation until later client approval
- Out of scope: Creating a new developer remediation group; guideline edits; commissioning another external review artifact

**Inputs Required**:
- TK010.4 fresh external review
- Refreshed AC000 package from TK010.3
- `prompt/artifacts/tasks/T102/workspace/PH001/ST002/AC000/proposal/proposal_T102-PH001-ST002-AC000_gate-001-disposition.md` -- current proposal surface to refresh
- `prompt/templates/consultant/workspace/guideline_workspace_proposal.md` -- proposal authoring rules

**Steps**:
1. Read the fresh external review and the refreshed AC000 package.
2. Assess the findings against the current GATE-001 proposal surface.
3. Refresh the proposal so it is ready for client re-presentation.
4. If any additional remediation is identified, record it only as subject to later client approval after consultant assessment.

**Success Criteria**:
- [ ] Proposal package is internally consistent for client re-presentation
- [ ] Any further remediation is deferred to later client approval after consultant assessment
- [ ] No new developer remediation group has been added
...
### Task TK011: Execute Implementation Per TK010 Downstream Execution Contract

**Task ID**: `T102-PH001-ST002-AC000-TK011`

**Purpose**: Execute the bounded AC000 implementation specified in `TK010` after the same-gate recycle loop is cleared through TK010.5 and GATE-001 is approved. This task remains blocked until that approval activates `TK010`.

**Deliverable**:
- Mutated STD file(s) and AC000 implementation surfaces per `TK010`

**Scope**:
- In scope: File mutations and bounded AC000 package updates specified in `TK010`
- Out of scope: Any mutations not specified in `TK010`

**Inputs Required**:
- `prompt/artifacts/tasks/T102/workspace/PH001/ST002/AC000/implementation/implementation_T102-PH001-ST002-AC000_tk010_gate-001-downstream-seed-task-specification.md` -- activated execution authority after `GATE-001` approval
- Target STD files and AC000 surfaces as specified in `TK010`

**Steps**:
1. Read `TK010` after the recycle loop clears and `GATE-001` approval activates it
...
**Purpose**: Capture bounded implementation evidence for the GATE-002 review package. The first cycle produces the primary DEV-REPORT; any recycle cycle produces supplementary DEV-REPORT artifacts under the same GATE-002 package. All supplementary cycles remain under the same GATE-002.
...
3. Include `implementation_reference` pointing to `TK010`
...
**Purpose**: Produce independent reviewer evidence for the GATE-002 review. The main LLM_Consultant performs the verification pass for this cycle, and any recycle cycle produces supplementary verification artifacts under the same GATE-002 package while the primary verification remains the gate-facing verdict surface.

**Deliverable**:
- `prompt/artifacts/tasks/T102/workspace/PH001/ST002/AC000/verification/verification_T102-PH001-ST002-AC000_gate-002.md`

**Steps**:
1. Read the dev-report (TK012) and `TK010`
2. Perform evidence-first verification per `guideline_workspace_verification.md` §V using the active primary DEV-REPORT and any linked supplementary recycle-cycle evidence; the main LLM_Consultant performs the verification pass for this cycle.
3. Verify each SPEC item's end-state posture against `TK010` and actual file state
...
**Purpose**: Package the GATE-002 decision items and prepare the GDR for client disposition. Proposal authoring may begin only after the latest verification cycle returns a non-RECYCLE verdict.
...
2. Evidence Index: reference the dev-report (TK012), verification (TK013), and `TK010` as primary evidence; list any supplementary recycle-cycle evidence second and historical evidence last.
...
**Purpose**: Produce an independent second-opinion quality audit of the GATE-002 gate package before client disposition. The external review consumes the authoritative primary package surfaces plus any linked supplementary recycle-cycle evidence.
...
**Scope**:
- In scope: Review of TK014 gate-disposition proposal; review of TK012 dev-report and TK013 verification; assessment of evidence integrity, role boundaries, plan compliance, and downstream readiness for AC001–AC004; inclusion of any linked supplementary recycle-cycle evidence under the same gate
...
**Entry Criteria**:
- TK011 exists and was executed from `TK010` after the recycle loop is cleared and `GATE-001` is approved
- TK012 dev-report captures execution evidence
- TK013 verification artifact exists with reviewer verdict
- TK014 gate-disposition proposal exists with populated GDR in pending state and indexes the external review plus final consultant assessment
- TK015 external review analysis exists with findings reviewed by main `LLM_Consultant`
- Final consultant assessment exists and is indexed in the gate package
- The active primary DEV-REPORT/VERIFICATION package may include linked supplementary recycle-cycle artifacts when the gate has recycled.

**Reviewer**: Client

**Exit Criteria**:
- Client records decision in the GDR per `guideline_workspace_proposal.md` §VII.D
- If APPROVE: AC000 implementation-backed gate closes; downstream activity packages may proceed only through their own governing activity plans and explicit commission, using calibrated scope brief + verified STD mutations as primary inputs where applicable
- If APPROVE WITH CONDITIONS: conditions are recorded and addressed before downstream activities are commissioned through their own plans
...
|:--|:--|:--|:--|
| v2.10.0 | 2026-04-02 | Amendment | Refreshed the GATE-002 task stack to reflect completed TK011-TK015 execution, moved GATE-002 to `in_progress` while the GDR remains pending, and aligned the gate section to the AC000-only downstream boundary with the final consultant assessment indexed. |
| v2.9.0 | 2026-04-02 | Amendment | Aligned the GATE-002 task stack to consultant-owned verification, primary/supplementary recycle-cycle evidence handling, and ordered Evidence Index guidance for same-gate package refreshes. |
...
| v2.7.0 | 2026-03-30 | Amendment | Repaired the AC000 downstream execution contract surface. `TK010` now remains inactive before gate approval and then acts as the bounded execution contract for `TK011`-`TK015`; task, gate, and GATE-002 text now reference `TK010` directly instead of an undefined future implementation spec. |
| v2.6.0 | 2026-03-30 | Amendment | Marked TK010.4 and TK010.5 completed after the fresh second external review and main-consultant proposal refresh were finished. Normalized the active TK010.4 artifact path to `...gate-001-recycle-external-review.md`, kept TK009 historical-only, and restored TK011 blocking to GATE-001. |
| v2.5.0 | 2026-03-30 | Amendment | Extended the GATE-001 recycle/re-presentation path with TK010.4 fresh second external review and TK010.5 consultant assessment/proposal refresh, then kept TK011-TK015 and GATE-002 blocked behind the expanded same-gate loop. |
