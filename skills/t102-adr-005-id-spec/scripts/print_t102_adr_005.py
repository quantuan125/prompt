...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Print only the T102-ADR-005 (ID Specification & Rules) block from the Concept "
            "document, so agents can follow the latest RID rules without loading the entire file."
        )
    )
    parser.add_argument(
        "--concept-path",
...
def main() -> int:
    ...
    try:
        ...
        print(f"ERROR: Failed to extract ADR-005 block: {exc}", file=sys.stderr)
        return 1
