...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Print only the T102-ADR-004 (Decision Records Index) block from the Concept "
            "document, so agents can follow the latest DR index rules without loading the entire file."
        )
    )
    parser.add_argument(
        "--concept-path",
...
def main() -> int:
    ...
    try:
        ...
        print(f"ERROR: Failed to extract ADR-004 block: {exc}", file=sys.stderr)
        return 1
